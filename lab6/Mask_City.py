...
import transforms
from CityDataset import CityscapesDataset
...
#%%
city_path = "/root/labs/Cityscapes"
dataset = CityscapesDataset(city_path,split='val',mode="fine", augment=False)

#%%
print('Loading Cityscapes train, val datasets...')

def collate_fn(batch):
    return tuple(zip(*batch))

city_dataloader = torch.utils.data.DataLoader(dataset, batch_size=4, shuffle=False, collate_fn=collate_fn)
# %%

city_set = city_dataloader.dataset
img ,targetmask = city_set[0]
print(targetmask.shape)   
# %%
images, targets = next(iter(city_dataloader))
