...
def train_model(model, dataloaders, criterion, optimizer, num_epochs=25, weights_name='weight_save', is_inception=False):
    ...
    best_acc = 0.0
    phase_info = { 'train': "Training", 'val': "Validating" }
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            for inputs, labels in tqdm(dataloaders[phase],desc=phase_info[phase]):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if is_inception and phase == 'train':
                        ...
                        loss2 = criterion(aux_output1, labels)
                        loss3 = criterion(aux_output2, labels)
                        loss = loss1 + 0.3 * loss2 + 0.3 * loss3
                    elif is_inception and phase == 'val':
                        outputs, aux_output1,aux_output2 = model(inputs)
                        loss1 = criterion(outputs, labels)
                        loss2 = criterion(aux_output1, labels)
                        loss3 = criterion(aux_output2, labels)
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                #torch.save(model.state_dict(), f"/root/labs/checkpoints/{weights_name}-{epoch}-bestacc{best_acc:.2f}sgd-0.02.pth")
            if phase == 'val':
    ...
    return model, val_acc_history, loss_acc_history,best_acc